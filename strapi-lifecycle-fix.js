/**
 * Lifecycle hooks para wo-pedido
 * Este archivo debe ir en: src/api/wo-pedido/content-types/wo-pedido/lifecycles.js
 * 
...
module.exports = {
  async afterUpdate(event) {
    ...
    try {
      ...
      const originPlatform = pedido.originPlatform || pedido.origin_platform;
      const wooId = pedido.wooId || (pedido.externalIds?.wooCommerce?.id);
      ...
      if (items.length > 0 && itemsValidos.length === 0) {
        ...
        console.warn('[wo-pedido] ⚠️ Detalles de items:', items.map(item => ({
          id: item.id,
          nombre: item.nombre,
