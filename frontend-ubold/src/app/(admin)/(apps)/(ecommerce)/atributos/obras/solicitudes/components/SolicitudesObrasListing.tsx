...
// Función para mapear solicitudes de Strapi al formato SolicitudObraType
const mapStrapiSolicitudToSolicitudType = (solicitud: any): SolicitudObraType => {
  ...
  const codigo = getField(data, 'codigo_obra', 'codigoObra', 'CODIGO_OBRA') || ''
  ...
  return {
    ...
    url: `/atributos/obras/${solicitud.id || solicitud.documentId || solicitud.id}`,
  }
}

interface SolicitudesObrasListingProps {
  solicitudes?: any[]
  error?: string | null
}

const columnHelper = createColumnHelper<SolicitudObraType>()

const SolicitudesObrasListing = ({ solicitudes, error }: SolicitudesObrasListingProps = {}) => {
  const router = useRouter()
  
  // Mapear solicitudes de Strapi al formato SolicitudObraType si están disponibles
  const mappedSolicitudes = useMemo(() => {
    if (solicitudes && solicitudes.length > 0) {
      console.log('[SolicitudesObrasListing] Solicitudes recibidas:', solicitudes.length)
      const mapped = solicitudes.map(mapStrapiSolicitudToSolicitudType)
      console.log('[SolicitudesObrasListing] Solicitudes mapeadas:', mapped.length)
      return mapped
    }
    console.log('[SolicitudesObrasListing] No hay solicitudes de Strapi')
    return []
  }, [solicitudes])

  const columns: ColumnDef<SolicitudObraType, any>[] = [
    {
      id: 'select',
      maxSize: 45,
      size: 45,
      header: ({ table }: { table: TableType<SolicitudObraType> }) => (
        <input
          type="checkbox"
      ...
      cell: ({ row }: { row: TableRow<SolicitudObraType> }) => (
        <input
          type="checkbox"
          className="form-check-input form-check-input-light fs-14"
    ...
    columnHelper.accessor('codigo_obra', {
      header: 'Código',
      cell: ({ row }) => (
        <span className="fw-semibold">{row.original.codigo_obra || '-'}</span>
      ),
    }),
    columnHelper.accessor('nombre_obra', {
      header: 'Obra',
      cell: ({ row }) => {
        return (
            ...
            <div>
              <h5 className="mb-0">
                <Link href={row.original.url} className="link-reset">
                  {row.original.nombre_obra || 'Sin nombre'}
                </Link>
              </h5>
  ...
  const [data, setData] = useState<SolicitudObraType[]>([])
  ...
  useEffect(() => {
    console.log('[SolicitudesObrasListing] useEffect - solicitudes:', solicitudes?.length, 'mappedSolicitudes:', mappedSolicitudes.length)
    setData(mappedSolicitudes)
    console.log('[SolicitudesObrasListing] Datos actualizados. Total:', mappedSolicitudes.length)
  }, [mappedSolicitudes, solicitudes])

  const table = useReactTable<SolicitudObraType>({
    data,
    columns,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      for (const solicitudId of idsToDelete) {
        const response = await fetch(`/api/tienda/obras/${solicitudId}`, {
          method: 'DELETE',
        })
        if (!response.ok) {
  ...
  if (hasError && hasData) {
    console.warn('[SolicitudesObrasListing] Error al cargar desde Strapi, usando datos disponibles:', error)
  }
        ...
        <Card className="mb-4">
          <CardHeader className="d-flex justify-content-between align-items-center">
            <div className="d-flex gap-2">
              <div className="app-search">
                <input
                  ...
                  placeholder="Buscar solicitud de obra..."
                  value={globalFilter ?? ''}
            ...
            <div className="d-flex gap-1">
              ...
              <Link href="/atributos/obras/agregar" passHref>
...
export default SolicitudesObrasListing
