...
export const metadata: Metadata = {
  title: 'Solicitudes de Autores - Intranet Almonte',
}
...
export default async function Page() {
  ...
  try {
    ...
    if (data.success && data.data) {
      ...
      console.log('[Autor Requests Page] Autores obtenidos:', autores.length)
    } else {
      error = data.error || 'Error al obtener autores'
      console.error('[Autor Requests Page] Error en respuesta:', data)
    }
  } catch (err: any) {
    error = err.message || 'Error al conectar con la API'
    console.error('[Autor Requests Page] Error al obtener autores:', err)
  }

  return (
    <Container fluid>
      <PageBreadcrumb title="Solicitudes de Autores" subtitle="Ecommerce" />
      <AutorRequestsListing autores={autores} error={error} />
    </Container>
  )
}
